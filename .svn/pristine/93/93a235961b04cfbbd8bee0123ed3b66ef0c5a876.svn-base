...
public class BestSignUtils {
     ...
     * @throws Exception
     */
    public static String getUrlByRsa(String developerId, String pem, String account,
                                     String imageName, Map<String, Object> data,
        ...
        String sign = String.format(
            "account=%sdeveloperId=%simageName=%srtick=%ssignType=rsa/openapi/v3%s/%s", account,
            developerId, imageName, rtick, path, dataMd5); // 生成签名字符串
        System.out.println(sign);

        String signDataString = getSignData(sign);
        String signData = Base64
            .encodeBase64String(EncodeUtils.rsaSign(signDataString.getBytes("UTF-8"), pem));

        signData = URLEncoder.encode(signData, "UTF-8");
        path = path + "/?developerId=" + developerId + "&rtick=" + rtick + "&sign=" + signData
               + "&signType=rsa";
    ...
    public static String getPostUrlByRsa(String developerId, String pem, Map<String, Object> data,
                                         String path) throws Exception {
        ...
        String sign = String.format("developerId=%srtick=%ssignType=rsa/openapi/v3%s/%s",
            developerId, rtick, path, dataMd5); // 生成签名字符串
        System.out.println(sign);
